...
const satisfaccion = function () {
  var puntajeDelCliente = parseInt(
    prompt("¿ Como nos calificarías del 1 al 5?")
  );

  switch (puntajeDelCliente) {
    case 1:
      alert(
        "Lamentamos mucho tu insatisfacción. Trabajaremos duro para mejorar."
      );
      break;
    case 2:
      alert(
        "Lamentamos tu mala experiencia. Estamos en aprendizaje continuo para sguir mejorando."
      );

      break;
    case 3:
      alert(
        "Te agradecemos mucho tu devolución, tu valoración nos ayuda a crecer."
      );

      break;
    case 4:
      alert(
        "Nos alegra que tu percepción sea buena, aceptamos toda sujerencia para ser mejores!"
      );

      break;
    case 5:
      alert(
        "Estamos muy felices de colmar tus expectativas! Gracias por puntuarnos!"
      );

      break;

    default:
      alert("Por favor ingresa un número de 1 a 5.");
      break;
  }
};
name();
...
alert(
  "Te vamos a informar los precios de cada producto sin IVA, (el cual es el 21% sobre el precio del producto) mas el costo de envío, el cual a su vez es un 5%"
);

alert(
  `Igual vos tranquil@, por que solo nos vas a decir que cantidad querés de cada producto y nosotros te informamos cual sería el valor total de tu compra!`
);

function compra() {
  let qDestapadores = parseInt(
    prompt(
      "El precio sin Iva del destapador es de $1000. Por favor ingresa cantidad de Destapadores a comprar"
    )
  );

  let qPad = parseInt(
    prompt(
      "El precio sin Iva del PadGamer es de $1500. Por favor ingresa cantidad de Pad's a comprar"
    )
  );

  let qLlaveros = parseInt(
    prompt(
      "El precio sin Iva del Llavero es de $300. Por favor ingresa cantidad de Llaveros a compar"
    )
  );

  let qHumidificadores = parseInt(
    prompt(
      "El precio sin Iva del Humidificador es de $1800. Por favor ingresa cantidad de Humidificadores a compar"
    )
  );

  let qSportbag = parseInt(
    prompt(
      "El precio sin Iva del destapador es de $650. Por favor ingresa cantidad de Riñoneras a compar"
    )
  );
...
class Product {
  ...
  agregarAlCarrito(cantidad) {
    ...
    alert(
      "acabas de sumar : " +
        cantidad +
        " y aún puedes solicitar " +
        this.stockActual +
        " unidades mas si deseas."
    );
  }
  ...
  precioFinal(cantidad) {
    ...
    alert(
      `Vas a comprar ${cantidad} unidades. El IVA por esta conmpra es de $${iva}. El costo de envío para este pedido es de $${costoEnvio}. Precio final: $${precioFinal}`
    );
  }
}
...
const simulationProduct2 = new Product(
  0001,
  "Pen Drive",
  "rojo",
  600,
...
const simulationProduct3 = new Product(
  0001,
  "Lapicera",
  "negro",
  700,
...
for (let i = 0; i < prices.length; i++) {
  console.log(
    `Producto: ${prices[i].nombre} - Precio: $${prices[i].precioSinIva}`
  );
}
